...
import { GUITAR_RECORDING_FIXTURES, GUITAR_SEQUENCE_FIXTURES } from './fixtures/guitar-recordings.js';

for (const fixture of GUITAR_RECORDING_FIXTURES) {
	test(`detects ${fixture.guitar} string ${fixture.stringNumber} as ${fixture.note}`, async ({ page }) => {
...
for (const fixture of GUITAR_SEQUENCE_FIXTURES) {
	test(`tracks the ${fixture.guitar} sequence without note flicker`, async ({ page }) => {
		const recordingPath = fileURLToPath(new URL(`./fixtures/guitar-recordings/${fixture.file}`, import.meta.url));
		...
		expect(displayedNotes).toEqual(fixture.expectedNotes);
	});
}
